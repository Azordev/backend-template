...
const DB_PASSWORD_DATA = existsSync(DB_PASSWORD_FILE)
  ? readFileSync(DB_PASSWORD_FILE, 'utf8').trim()
  : undefined
const JWT_SECRET_FILE = process.env.JWT_SECRET_FILE || undefined
const JWT_SECRET_DATA = existsSync(JWT_SECRET_FILE)
  ? readFileSync(JWT_SECRET_FILE, 'utf8').trim()
  : undefined
...
export const JWT_SECRET = JWT_SECRET_DATA || process.env.JWT_SECRET || 'secret'
export const JWT_EXPIRE = Number(process.env.JWT_EXPIRE) || 14 * 24 * 60 * 60

export const FILE_IMAGE_SIZE =
  Number(process.env.FILE_IMAGE_SIZE) || 20 * 1024 * 1024
